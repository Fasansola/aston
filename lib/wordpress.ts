...
export async function createWordPressPost(
  ...
  // ── Yoast meta: fire-and-forget PATCH ───────────────────────
  // Intentionally NOT awaited — Yoast meta is non-critical SEO metadata.
  // Awaiting this was causing the Vercel 300s function limit to be hit
  // just before the SSE "done" event could be sent, making every generation
  // appear to fail even though the post was fully created.
  axios.patch(
    ...
    .then(() => console.log(`[wordpress] Yoast meta patched for post ${postId}`))
